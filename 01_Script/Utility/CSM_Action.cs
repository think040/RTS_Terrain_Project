...
public class CSM_Action : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        RenderGOM.PreRenderCSM += PreRenderCSM;
        RenderGOM.PostRenderCSM += PostRenderCSM;
    }
    ...
    {
        {
            csmRect = new Rect(0.0f, 0.0f, csmH, csmW);
        }
...

        {
            ...
            for (int i = 0; i < csmCount; i++)
            {
                if (bDepth)
                {
                    tex2D[i] = new Texture2D(csmW, csmH, TextureFormat.RFloat, false);
                }
                else
                {
                    tex2D[i] = new Texture2D(csmW, csmH, TextureFormat.ARGB32, false);
                }
            }
        }
